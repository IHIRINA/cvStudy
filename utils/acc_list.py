...
def excel_dict(root, keys, mode, save_data=None):
    if mode == 'read':
        ...
        df.to_excel(root, index=False)  # 存表，去除原始索引列（0,1,2...）
...
class TestImage:
    ...
    def make_list(self, loss, batch_idx, len_, t_v):  # 每个batch计算一次指标
